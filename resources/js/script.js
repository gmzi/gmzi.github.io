...
// Buttons //
    ...
    $('.js--scroll-to-dogs').click(function () {
        $('html, body').animate({scrollTop: $('.js--section-dogs').offset().top}, 1000);
    });
    ...
    buttonDogs.addEventListener('click',() => {
        ...
        .then(data => { if (data.url.includes(".mp4" || ".webm")) {
          frameImgDogs
        .innerHTML = `<video controls autoplay loop muted src=${data.url} type="video/mp4" class="dog-img"></video>`;
        } else {
          frameImgDogs
      ...
      buttonCats.addEventListener('click',() => {
        async function getCats() {
          const dataFecth = await fetch('https://api.thecatapi.com/v1/images/search', {
            method: 'GET',
            headers: {
